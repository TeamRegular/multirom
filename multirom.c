...
#ifndef MR_ALLOW_NKK71_NOKEXEC_WORKAROUND
    if(((M(type) & MASK_KEXEC) || to_boot->has_bootimg) && type != ROM_DEFAULT && s->is_second_boot == 0)
#else
    if(((M(type) & MASK_KEXEC) || (to_boot->has_bootimg && (nokexec()->selected_method == NO_KEXEC_BOOT_KEXEC))) && type != ROM_DEFAULT && s->is_second_boot == 0)
#endif
    {
...
int multirom_process_android_fstab(char *fstab_name, int has_fw, struct fstab_part **fw_part)
{
    ...
    else
    {
        ...
        while((dp = readdir(d)))
        {
            if(strstr(dp->d_name, "fstab.") == dp->d_name && strcmp(dp->d_name, "fstab.goldfish") != 0 && strcmp(dp->d_name, "fstab.ranchu") != 0 && strcmp(dp->d_name, "fstab.ftm.qcom") != 0)
            {
